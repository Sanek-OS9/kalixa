 ...
 */
/*
$q = DB::me()->prepare("SELECT `paymentID`,`merchantTransactionID` FROM `orders` WHERE `user_id` = ? AND `paymentID` IS NOT NULL AND `merchantTransactionID` IS NOT NULL LIMIT 1");
...
if ($order = $q->fetch()) {
  ...
  echo '<form action="pay.php?ven=' . $payVen . '" method="POST">
    <input name="paymentID" type="hidden" value="' . $order['paymentID'] . '" />
    <input name="merchantTransactionID" type="hidden" value="' . $order['merchantTransactionID'] . '" />
...
<div class="checkout">
  ...
  <form class="form" action="pay.first.php?ven=<?= $payVen ?>" method="POST" autocomplete="off" novalidate>
    <fieldset>
      ...
      <input type="num" id="card-number-3" class="input-cart-number" name="card-number[]" value="1111" maxlength="4" />
    </fieldset>
    <fieldset>
